...
class MultiStepSEWResNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # self.STSA1 = STSA(dim = 128,T = 8, H = 32 , W = 16)
        # self.STSA2 = STSA(dim = 512,T = 8, H = 16 , W = 8)
    ...
    def _forward_impl(self, x: torch.Tensor):
        ...
        # x_seq = self.STSA1(x_seq) + x_seq
        # x_seq = self.STSA1(x_seq)
        # print(x_seq.shape)
        ...
        _, _, c2, h2, w2 = x_seq.size()

        x_seq = x_seq.view(t1 * b1, c2, h2, w2)
        x_seq = F.max_pool2d(x_seq, x_seq.size()[2:])
        x_seq = x_seq.view(b1, t1, -1)
        x_seq = x_seq.mean(1)
        f = self.bn(x_seq)
        # x_seq = self.STSA2(x_seq) + x_seq
        # x_seq = self.STSA2(x_seq)
        # print(x_seq.shape)
...
def _multi_step_sew_resnet(
    ...
    model = MultiStepSEWResNet(
        block, layers, T=T, cnf='ADD', multi_step_neuron=multi_step_neuron, **kwargs
    )
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch], progress=progress)
